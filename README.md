...
```python
# Ví dụ 1: Khử nhiễu ảnh có sẵn
main("./images/vetinh.webp")

# Ví dụ 2: Khử nhiễu ảnh mẫu tự tạo
main()
```

Chương trình sẽ thêm nhiễu vào ảnh với ba mức độ khác nhau (σ = 10, 15, 20) và so sánh hiệu suất của ba phương pháp khử nhiễu.
...
- Bamberger, R. H., & Smith, M. J. (1992). A filter bank for the directional decomposition of images: Theory and design. *IEEE Transactions on Signal Processing*.
- Chang, S. G., Yu, B., & Vetterli, M. (2000). Adaptive wavelet thresholding for image denoising and compression. *IEEE Transactions on Image Processing*.
